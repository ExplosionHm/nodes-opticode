...
interface GoSchema {
  create(builder: fb.Builder, data: any): number;
}
...
export class GoBuilder<T extends GoSchema> {
  ...
  /* private async loadSchema(schemaName: string) {
    // Safely import schema by name
    try {
      const module = await import(`./schemas/${schemaName}_schema.js`);
      this.schema = module.default as T;
    } catch (err) {
      console.error("Error loading schema:", err, "Is the schema defined?");
    }
  } */
  ...
  constructor(private options: GoBuilderOptions) {
    ...
    this.stringTable.set(-1, "");
  }

  public SetNode(node: Node, id?: number): number {
    //! CLEAN THIS FUNCTION
  ...
  /* public SetConstNode(constants: Map<string, string | number>): Node {
    ...
    constants.forEach((v, k) => {
      nodes.push(this.CreateConstValueNode(k, v));
    });
  ...
  public CreateConstValueNode(name: string, value: string | number): Node {
    return {
      opcode: tree.Opcode.Const,
      left: {
        value: name,
  ...
  public Build(flags: number = 0): Uint8Array {
    ...
    const programName = this.builder.createString(options.name || defaultName);
